...
- **内置前端控制台** — 浏览器可直接完成 Freebuff 登录并提取 `AUTH_TOKENS`。
- **运行时 Token 注册** — 无需预先配置 `AUTH_TOKENS`，登录后自动注入并生效。
...
服务启动后访问 `http://<host>:8080/` 即可打开控制台页面，已融合 `freebuff_login_and_print.py` 的核心流程：

- 可选：配置 `ADMIN_PASSWORD` 后，页面先进行密码登录
- `POST /api/login/session`：创建登录会话并返回 `login_url`
...
- 控制台建议直接访问 `/` 或 `/ui`；`/ui` 已做无尾斜杠重定向处理，降低代理改写导致循环跳转的风险。
...
docker run -d --name Freebuff2API \
  ...
  ghcr.io/<你的GitHub用户名或组织>/freebuff2api:latest
```
...
项目内置 `.github/workflows/docker.yml`，会在以下场景自动构建并推送多架构镜像（amd64/arm64）到 GHCR：

- 推送到 `main`
- 推送 `v*` 标签
- 手动触发 `workflow_dispatch`

推送地址格式：`ghcr.io/<owner>/<repo>:latest`（默认分支）及 `ghcr.io/<owner>/<repo>:<tag>`（标签构建）。
